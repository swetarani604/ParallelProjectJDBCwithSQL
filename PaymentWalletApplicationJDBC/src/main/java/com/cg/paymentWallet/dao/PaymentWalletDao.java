...
public class PaymentWalletDao implements IPaymentWalletDao {
	...
	public double withdraw(String mobileNo, double withdrawAmount)
		...
		try {
			con.setAutoCommit(false);
			statement = con.prepareStatement(IQueryMapper.GET_ALL_DETAILS);
			...
			if (res != null) {
				...
				if (res.getDouble("balance") < withdrawAmount) {
					...
					acc.setCustomerName(res.getString("customerName"));
					acc.setEmailID(res.getString("emailId"));
					acc.setMobileNo(res.getString("mobileNo"));
					acc.setBalance(balance);
					statement1 = con.prepareStatement(IQueryMapper.UPDATE);
